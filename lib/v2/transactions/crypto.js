...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // qredit = 0x3a, devnet = 0x4B
...
function verify(transaction, network) {
	network = network || networks.qredit;
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.qredit);
