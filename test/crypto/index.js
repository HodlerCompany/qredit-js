...
describe("crypto.js", function () {
  ...
  describe("#getAddress", function () {
    ...
    it("should generate the same address as ECPair.getAddress() - DevNET", function () {
      ...
      var Q = ecurve.Point.decodeFrom(curve, new Buffer(keys.publicKey, 'hex'))
      var keyPairD = new ECPair(null, Q, { compressed: true, network: qreditjs.networks.devnet });
...
describe("different networks", function () {
  ...
  it("validate address on devnet should be ok", function () {
    qreditjs.crypto.setNetworkVersion(0x4B);
    qreditjs.crypto.getNetworkVersion().should.equal(0x4B);
    var validate = qreditjs.crypto.validateAddress("XE5npfYGHcnwsHZXFagNAJN6FFyAM542Bh");
    (validate).should.equal(true);
    qreditjs.crypto.setNetworkVersion(0x37);
    qreditjs.crypto.getNetworkVersion().should.equal(0x37);
  });
});

describe("delegate.js", function () {
  var delegate = qreditjs.delegate;
  ...
  describe("#createDelegate", function () {
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = qreditjs.crypto.fromBytes(qreditjs.crypto.getBytes(trs).toString("hex"));
    ...
    describe("returned delegate", function () {
      var keys = qreditjs.crypto.getKeys("secret");
      var secondKeys = qreditjs.crypto.getKeys("secret 2");
      ...
      it("should be signed correctly", function () {
        var result = qreditjs.crypto.verify(trs);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = qreditjs.crypto.verifySecondSignature(trs, secondKeys.publicKey);
        (result).should.be.ok;
      });
