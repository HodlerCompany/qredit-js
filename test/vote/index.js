...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = qreditjs.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = qreditjs.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = qreditjs.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.qredit);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = qreditjs.crypto.fromBytes(qreditjs.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(qreditjs.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = qreditjs.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = qreditjs.crypto.verifySecondSignature(vt, qreditjs.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
